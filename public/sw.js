const CACHE_NAME = 'voicebridge-v1.0.0';
const STATIC_CACHE = 'voicebridge-static-v1.0.0';
const DYNAMIC_CACHE = 'voicebridge-dynamic-v1.0.0';
...
function openIndexedDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('VoiceBridgeDB', 1);
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'New notification from VoiceBridge',
    icon: '/favicon-ico',
    badge: '/favicon-ico',
    vibrate: [100, 50, 100],
    data: {
  ...
  event.waitUntil(
    self.registration.showNotification('VoiceBridge', options)
  );
});
